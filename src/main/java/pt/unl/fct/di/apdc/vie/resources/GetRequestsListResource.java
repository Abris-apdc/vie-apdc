...
public class GetRequestsListResource {

	private final Datastore datastore = DatastoreOptions.getDefaultInstance().getService();
	private final Gson g = new Gson();
	
	public GetRequestsListResource() {
	}
	...
	public Response getFollowers(@PathParam("username") String username)  {
		...
		try {
			...
			List<Value<String>> requests = account.getList("account_requests_list");
			List<String> allResults = new ArrayList<>();
			
			for(int i = 0; i<requests.size();i++) {
				allResults.add(requests.get(i).get());
			}
			txn.commit();
